class MessageVariables < Mustache
  ...
  def method_missing(method_name, *arguments, &block)
    variable = MessageVariables.variables[method_name.to_s]
  ...
  def respond_to_missing?(method_name, include_private = false)
    MessageVariables.variables.key?(method_name.to_s) || super
  end
  ...
  class << self
    ...
    def reload_config
      @config = YAML.load_file("config/message_variables.yml")
    end
  end
